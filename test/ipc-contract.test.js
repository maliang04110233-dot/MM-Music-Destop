...
test('通道数量钉死（意外增删即失败，逼迫改动者过目契约）', () => {
  assert.strictEqual(MAIN_INVOKE.size, 99); // 2026-09 余项：-flush-prefs -flush-history（无人调用的僵尸通道）
  ...
  assert.strictEqual(Object.keys(METHODS).length, 103); // 余项：-windowToggleFullscreen -closeDesktopLyric（死键）
});
