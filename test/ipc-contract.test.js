...
test('通道数量钉死（意外增删即失败，逼迫改动者过目契约）', () => {
  assert.strictEqual(MAIN_INVOKE.size, 94); // +set-queue-paused +system-power +probe-audio
  ...
  assert.strictEqual(Object.keys(METHODS).length, 103); // +setQueuePaused +systemPower +probeAudio
});
